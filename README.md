...
Papers and parsed output are stored in Google Drive (`/content/drive/MyDrive/RAG/`) rather than this repo, due to size and copyright. See [docs/corpus_notes.md](../docs/corpus_notes.md) for known corpus gaps and hard detection ceilings.
...
│   └── parsed_output/                        # 30 paper JSONs + quality summary (or gitignored, linked via Drive)
│      
├── benchmark/                                # QA pairs + schema documentation
│   ├── qa_pairs.json                         # final verified ~185 QA pairs
...
## Results
The [Results](../results) folder contains the evaluation outputs comparing structure-aware vs. fixed-size chunking, at the per-QA-pair level. See [docs/methodology.md](../docs/methodology.md) for how these numbers were produced, and [benchmark/qa_schema.md](../benchmark/qa_schema.md) for the underlying QA pair schema.
...
- Answer quality follows the same pattern, except `faithfulness` and `multi_hop_comparative` do not show a statistically significant difference between strategies — traced to a **joint top-5 coverage gap** rather than a chunking-strategy weakness (see [docs/methodology.md](../docs/methodology.md)).
...
## Known Limitations
See [docs/corpus_notes.md](../docs/corpus_notes.md) for details on:
